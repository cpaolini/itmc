...
if len(sys.argv) != 5:
    print("Usage: python script.py <cameraID> <password> <count> <jetson_sudo_password>")
...
try:
    ...
    print("Camera ID and count must be integers.")
    sys.exit(1)
...
# --- Sync system clock BEFORE recording ---
print("Syncing system clock...")
try:
    # Use echo to provide sudo password to ntpdate
...
output_filename = f"camera_{cameraID}_video_{count}.mp4"
...
print(f"Recording 30 minutes of footage from camera {cameraID} with auto-reconnect...")
...
while time.time() - start_time < 1800:
    ...
    if not ret:
        ...
        if read_failures >= max_read_failures:
            ...
            break
        continue
    read_failures = 0  # reset failure counter on success
...
# Save timestamps to .txt file
timestamp_filename = output_filename.replace(".mp4", "_times.txt")
...
# Start SCP process in the background
...
# Periodically check remote file size
