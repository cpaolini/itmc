...
while True:
    ...
    display_frame = None
    # --- New: For each camera, fill polygons in world grid and count overlaps ---
    cam_polygons = []
    ...
    # For each camera, create a binary mask of its polygons in the world grid
    cam_masks = []
    ...
    # Sum masks to get overlap count
    for mask in cam_masks:
        overlap_count += mask
    # Visualize: blend green for detected areas
    for mask in cam_masks:
        idx = mask > 0
        world_grid[idx] = (0.75 * world_grid[idx] + 0.25 * np.array([0,255,0])).astype(np.uint8)
    # Assign points to each pixel based on overlap count using user-defined values
    vectorized_points_func = np.vectorize(points_for_overlap_count)
    points_grid = vectorized_points_func(overlap_count)
    # Create a heatmap-like image for World Grid 2 with shades of red
    ...
    for cnt in contours:
        ...
        area = w_exp * h_exp
        # Snap angle if within threshold (unless area > RECTANGLE_MAX_AREA or aspect ratio < RECTANGLE_SNAP_MIN_RATIO)
        snapped = False
        angle_mod = angle % 180
        ...
        if allow_snap:
            ...
            snap_angle = angle
        # Create new rect with expanded size and snapped angle if needed
        ...
        if area < RECTANGLE_MIN_AREA:
            # Check if all vertices are within RECTANGLE_EDGE_DIST_PX of the edge
            ...
            # Find which side is closest to edge and if it's the long side
            if close_to_edge:
                # Find distances to each edge for each vertex
                ...
                # If the closest side is the long side, extend the short side to the edge
                if ((edge_axis in [2,3] and w_exp >= h_exp) or (edge_axis in [0,1] and h_exp > w_exp)):
                    # For horizontal rectangle (wider than tall)
                    if w_exp >= h_exp:
                        if edge_axis == 3:  # bottom
                            # Set y=max for both bottom corners
                            max_y = np.max(box[:,1])
                            for i in range(4):
                                if box[i][1] == max_y:
                                    box[i][1] = grid_scale-1
                        elif edge_axis == 2:  # top
                            # Set y=0 for both top corners
                            min_y = np.min(box[:,1])
                            for i in range(4):
                                if box[i][1] == min_y:
                                    box[i][1] = 0
                    # For vertical rectangle (taller than wide)
                    else:
                        if edge_axis == 0:  # left
                            # Set x=0 for both left corners
                            min_x = np.min(box[:,0])
                            for i in range(4):
                                if box[i][0] == min_x:
                                    box[i][0] = 0
                        elif edge_axis == 1:  # right
                            # Set x=max for both right corners
                            max_x = np.max(box[:,0])
                            for i in range(4):
                                if box[i][0] == max_x:
                                    box[i][0] = grid_scale-1
        # --- Clause 3: Split large, nearly-square rectangles if a low-density slot exists ---
        split_done = False
        if area > RECTANGLE_MAX_AREA:
            side_ratio = max(w_exp, h_exp) / min(w_exp, h_exp)
            if side_ratio < RECTANGLE_SQUARE_RATIO:
                # Try to split along both axes
                for axis in [0,1]:
        ...
        if coverage >= 0.4:
            ...
            # Print rectangle info: length, width, angle, area, color, and corners
            rect_color = 'orange' if snapped else 'red'
            corners = [tuple(pt) for pt in box]
            print(f"Rectangle ({rect_color}) - Length: {max(w_exp, h_exp):.2f}, Width: {min(w_exp, h_exp):.2f}, Angle: {snap_angle:.2f} deg, Area: {w_exp * h_exp:.2f}, Corners: {corners}")
    # Optionally: Draw rectangles for each car polygon (in blue)
    for polygons in cam_polygons:
        for poly in polygons:
            if len(poly) >= 3:
                ...
                box = np.int0(box)
                cv2.drawContours(world_grid, [box], 0, (255, 0, 0), 1)
    # --- Display windows as before ---
    ...
    cv2.imshow("World Grid 2 (Overlap Points Heatmap)", World_Grid_2)
